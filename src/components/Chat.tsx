...
export default function Chat() {
  ...
  const [messages, setMessages] = useState<Array<Message>>(conversation?conversation.messages:[]);
  ...
  const handlePredict = async () =>{
    ...
    if (!value) return
    setMessages(prev=>([...prev,  {content:value, time: new Date(),user:"user"}]))
    ...
    setMessages(prev=>([...prev, {content:resJson, time: new Date(), user:"ai"}]))
    setLoading(false)
  }
  ...
  const addHistoryItem = () => {
    setState((prev)=>{
      ...
      return ({...prev, currentConversation: newHistory as History, history: [...history, newHistory ] as History[]})
    })
  }
  ...
  return (
    <div className="flex w-full flex-col items-center justify-between h-full p-5">
      ...
      <div className="w-full grow border-2 rounded-md my-5 shadow flex-col p-4 justify-end space-y-3 overflow-scroll h-1 flex-nowrap">
        {messages && messages.map((msg,i)=>(<div key={`key-${i}`} className={"border p-3 w-fit max-w-[80%] rounded-md" +(msg.user =="user"?" ml-auto bg-blue-200":"bg-gray-200")}>
          {msg.content}
        </div>))}
